...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN: PIRES\n' // GANTI NAMA LU
            + 'ORG: Pires Bot;\n'
            + 'TEL;type=CELL;type=VOICE;waid=5596103915:+55 69 9610-3915\n' // GANTI NOMOR LU
            + 'END:VCARD'
// JÁ CONSOLE ENTENDENDO SÓ VAMOS
...
baby.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `[ *BEM VINDO AO GRUPO ${mdata.subject}* ] \n___________________________\n@${num.split('@')[0]} Apresentacao/Membro!!! \n➸ Nome : \n➸ Idade : \n➸ Ja Transa? : \n➸ Recebe Bolsa Familia: \n➸ Namora/Solteiro: \n➸ Vai Seguir As Regras Deste Grupo?!!! \n➸ Instagram? \nNAO INVADA O PV DOS MEMBROS! \n *___________________________*\nSeja bem vindo a este grupo🛀!!`
				...
				teks = `ESTAMOS PERDENDO CORNOS... @${num.split('@')[0]}👋* \n_Adeus espero te encontrar de novo em um estado melhor_`
				let buff = await getBuffer(ppimg)
				baby.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
// ANTI LINK GRUP
		        ...
		        reply(`Adeus ${sender.split("@")[0]} Nao Pode Link fi de rapariga😡`)
		        setTimeout( () => {
...
*/
				...
				if (isBanned) return reply(nad.baned())
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(`Envie fotos / videos / gifs com legendas \n${prefix}sticker (durasi sticker video 1-9 detik)`)
					}
					break
				case 'nulis':
                  if (isBanned) return reply(nad.baned())
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (args.length < 1) return reply(`Cadê o texto mana? Contoh : ${prefix}nulis Ramlan baik hati`)
				nul = body.slice(7)
				reply('「❗」ESPERE BRO ')
				...
				ramlan = body.slice(8)
				reply('「❗」ESPERE BRO GUE NULIS DUMLU YAKAN')
		...
		case 'textlight':
		    ...
		    break
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					if (args.length < 1) return reply(`O que voce quer perguntar?? Exemplo : ${prefix}simi halo`)
					teks = body.slice(5)
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (args.length < 1) return baby.sendMessage(from, 'Onde está o código do idioma Sis? exemplo : ${prefix}tts id Olá PIRES', text, {quoted: mek})
					...
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('O texto é muito longo man')
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (!isQuotedSticker) return reply('Responda ou marque o adesivo que você deseja fazer uma foto  >_<')
					reply(nad.wait())
...
// PREMIUM
			  ...
			  case 'mutual':
                ...
                await reply('À procura de um parceiro >_<')
            ...
            case 'next':
				...
				if (!isPrem) return reply(nad.premium())
					...
					reply('「❗」AMOR Pausa 1 MINUTO ESTE SIM, IRMÃO')
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(nad.stikga())
		 ...
		 case 'tomp3':
				...
				if (!isPrem) return reply(nad.premium())
                	baby.updatePresence(from, Presence.composing) 
					if (!isQuotedVideo) return reply('*Responder ao vídeo-_-*')
					reply(nad.wait())
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await baby.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('Yahh falhou um:(')
				...
				if (!isPrem) return reply(nad.premium())
					...
					reply('「❗」PAUSA AMOR A 1 MINUTO DESTE SIM, IRMÃO ')
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(nad.stikga())
				...
				if (!isPrem) return reply(nad.premium())
					...
					reply('「❗」AMOR Pausa 1 MINUTO DESTE SIM, IRMÃO ')
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(nad.stikga())
				...
				case 'yutubdl':
					...
					teks = `*➸ JUDUL* : ${lah.title}\n\n*[WAIT] Processo de floração do colar*`
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply('SUA URL NÃO É VÁLIDA, IRMÃO')
					anu = await fetchJson(`https://api.zeks.xyz/api/tiktok?url=${args[0]}&apikey=apivinz`,)
					reply('[WAIT] Processo em andamento')
...
*/
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					const bisa =['Claro que eu posso! Você é a pessoa mais caseira ',' Não pode ser aowkwowk ',' Hmm, não sei Yaa, pergunte ao seu pai ',' Repita Tod que não entendo ']
...
case 'neontext':
		   ...
		   await limitAdd(sender)
                ...
                if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					const trut =['Já gostou de alguém? quanto tempo? ',' Se possível ou se mau, na gc / fora da gc quem você faria seu amigo te Alesanya?','Quais são as coisas que te fazem feliz quando você está triste? ',' Você já amou não correspondido? se alguma vez com quem? como se sente, brou? ',' as pessoas já foram traídas? ',' a coisa que é mais amigos e ',' quem é a pessoa que tem mais influência na sua vida ',' o orgulho que você teve este ano ',' quem é a pessoa que pode te deixar desconfortável ',' quem é a pessoa que sempre te deixou desconfortável ',const ttrth = trut[Math.floor(Math.random() * trut.length)]
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))                
					const dare =['Envie uma mensagem para o seu ex e diga "Eu ainda gosto de você ',' ligue para a namorada / namorada agora e ss para o jogador ',' papai para um dos membros do grupo ',' Diga" VOCÊ É TÃO BONITA, NÃO É FODA "para pessoal ',' ss const der = dare[Math.floor(Math.random() * dare.length)]
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				const bapak =['Nossa, você ainda tem o Bapack! Awowkwowk \ nPaciente Bro Don't Have Bapack \ n # Camda ',' Your Bapack Bjir Turns Out const cek = bapak[Math.floor(Math.random() * bapak.length)]
				...
				setTimeout( () => {
				reply("Woyy acorda está ficando sem tempo :v")
				}, timer)
				break						   
/*
]=====> MENU GRUP <=====[
*/		 
				case 'welcome':
                  if (isBanned) return reply(nad.baned())				
					...
					if (args.length < 1) return reply('Ta ativo >_<')
					if (Number(args[0]) === 1) {
				...
				case 'nsfw':
                  if (isBanned) return reply(nad.baned())				
					...
					if (Number(args[0]) === 1) {
						...
						reply('*「 SUCESSO 」ATIVAR OS RECURSOS NSFW NO GRUPO*')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/group/nsfw.json', JSON.stringify(nsfw))
				...
				if (!isRegistered) return reply(nad.noregis())	
					...
					try {
						...
						console.log('Error :', e)
						reply('Anjim que quer ser adicionado em privado, vamos :) ')
}
					break
			     	case 'kick':
                  if (isBanned) return reply(nad.baned())
				if (!isRegistered) return reply(nad.noregis())	     	
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Pedido aceito removido deste grupo , motivo muito claro estava usando calcinha bege e isto e broxante🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						baby.groupRemove(from, mentioned)
					} else {
						mentions(`Pedido aceito removido deste grupo , motivo muito claro estava usando calcinha bege e isto e broxante @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						baby.groupRemove(from, mentioned)
					}
					break
                case 'hidetag':
           ...
           case 'demote':
                if (!isRegistered) return reply(nad.noregis())           
					...
					if (mentioned.length > 1) {
						...
						mentions(`Yahh @${mentioned[0].split('@')[0]} Sua posição como ancestral no grupo foi removida🏃`, mentioned, true)
						baby.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
...
case 'ytmp3':
     ...
     if (anu.error) return reply(anu.error)
     ingfomp3 = `*「❗」Cancao Encontrada「❗」*\n➸  : $ {Album : ${anu.title}\n\n*[ESPERA CORNO] Processo Em Andamento*`
...
*/
                ...
                addKoinUser('556996103915@s.whatsapp.net', fee)
				...
				const koinPerlimit = 100000000000
...
*/
                ...
                if (isLimit(sender)) return reply(nad.limitend(pusname))
						...
						baby.sendMessage(from, nikko, image, {quoted: mek, caption: 'Don't Comly >_<'})
					await limitAdd(sender)
					break					
                case 'nekonime':
                if (isBanned) return reply(nad.baned())
...
*/				
            ...
            case 'creator':
                  ...
                  baby.sendMessage(from, 'Esse é o número do meu namorado >_<, Ehh Ownerku mksdnya:v',MessageType.text, { quoted: mek} )
					break
case 'hartatahta':
if (isBanned) return reply(nad.baned())
if (!isRegistered) return reply(nad.noregis())
...
cloud = body.slice(11)
reply('「❗」Espere mano, eu vôo primeiro')
...
*/
                case 'addprem':
					...
					reply(`Sucesso adicionado ${addp} Ke Daftar Premium`)
					break
				case 'dellprem':
					if (!isOwner) return reply(nad.ownerb())
					oh = body.slice(11)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 SCREAMO TRANSMISSAO 」*\n\n${body.slice(4)}`)
						}
						reply('*「 MEU BOT SUCESSO 」*')
					}
					break
				case 'bcgc':
				     if (!isOwner) return reply(nad.ownerb())
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						reply('*「 SUKSES TRANSMITIR 」*')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 BC GROUP 」*\n\n➸ Dari Grup : ${groupName}\n➸ Pengirim : wa.me/${(sender.split('@')[0])}\n➸ Pesan : ${body.slice(6)}`)
						}
						reply('*「 SUCESSO TRANSMISSAO 」*')
					}
					break
					case 'setreply':
					if (!isOwner) return reply(nad.ownerb())
